...
function drawChart(dataset){
    ...
    // groupedData.forEach(function(row, index) {
        // console.log(row)

        // // from any to 1
        // Doro Sando, Mayer Berta
        // Amerigo Tot, Gulacsy Lajos, Kiss Zoltan
        // Huller Agoston, Kemeny Judit, Szabo Tamas
        // Toth Gabor, Kovacs Zoltan, Szabo Dorottya
        // Raboczky Judit Rita, Deim Balazs, Gora Orsolya

        // // from any to 2
        // Szekely Beata, Karolyi Erno
        // Egyed Laszlo, Orosz Peter, Szunyoghy Viktoria
        // Hollo Istvan, Boros Viola, Erdos Gabor
        // Szepessy Bela, Veszely Beata, Nagy Attila
        // Kiss Adel, Dechandt Antal, Zsaki Istvan
        // Ruttkay Sandor, Jovanovics Gyorgy
    ...
    linesOne.exit()
    .transition().delay(1500 / 2).duration(1500 / 2)
    .attr("opacity", 0)
    .remove()
    ...
    //  curvedTextsOrderList.forEach(el => {
        ...
        d3.json("../data/grouped-with-curve-number.json").then(data => {
        ...
        curveTextsEnter.append("textPath")
                        .attr("xlink:href", (d, i) => { 
                            ...
                            for (let el = 0; el < groupedData.length; el++){
                                if ( d.artist == groupedData[el].key) {
                                    const lineNum = +groupedData[el].row_number + 1;
            ...
            curveTexts.exit()
            .transition().delay(1500 / 2).duration(1500) 
            .attr("opacity", 0)
            .remove()
